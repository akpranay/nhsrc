...
$dat =$date."/".$month."/".$year;  
      ...
      $sel="select uid from final_preg where uid = '$uid'";
   ...
   if($hname==NULL OR $uid==NULL OR $dname==NULL OR $dat==NULL OR $prob==NULL OR $pres==NULL )
     {
              
               $errMSG= "Sorry Enter required Details ...";
     } 
     else if  ($count == 0)
     {
           
           $errMSG= "Sorry This UID is not registered ...";
     }
     
     else
     {
                      
          $sql= "INSERT INTO final_treat (uid, hosname, docname, check_dt, prob, test, report, pres)
            VALUES ('$uid', '$hname', '$dname', '$dat', '$prob', '$chk', '$rep', '$pres')";
            
           if(mysqli_query($link,$sql)){
           
                  $emg = "records added successfully";
           }
   ...
   /*if($count==0  )
    {
       if($uname!=NULL && $city!=NULL && $state!=NULL && $email!=NULL && $pass!=NULL && $hname!=NULL && $chk!=NULL && $cname!=NULL && $typeg!=NULL && $category!=NULL && $hrg!=NULL && $website!=NULL && $count2==1 )
        {
$sql = "INSERT INTO final_hreg(huname,hpass,hosname,state,city,type,ceo,gov,gen,reg,website,email)
VALUES ('$uname','$pass','$hname','$state','$city','$chk','$cname','$typeg','$category','$hrg','$website','$email')";
echo $sql; 
if(mysqli_query($link,$sql)){
...
$hname = $hrg = $email = $website = $uname = $state = $city = $cname = $typeg = $typet = $category = "";?>
 ...
 <?php
            if ( isset($errMSG) ) {
                ...
                <a href="treat.php"><h3>Go to registration</h3></a>
                <?php
            }
            else if(isset($emg)){
                ?>
