...
$spass = hash('sha256', $pass);
   ...
   $sel="select huname from final_hreg where huname = '$uname'";
   $result = mysqli_query($link,$sel);
    $count = mysqli_num_rows($result); // if username not found then proceed
    
       $sel2="select reg_no from gov_hreg where reg_no = '$hrg'";
   $result2 = mysqli_query($link,$sel2);
    $count2 = mysqli_num_rows($result2);
    
    $sel3="select reg from final_hreg where reg = '$hrg'";
   $result3 = mysqli_query($link,$sel3);
    $count3 = mysqli_num_rows($result3);
   
   if($uname==NULL OR $city==NULL OR $state==NULL OR $email==NULL OR $pass==NULL OR $hname==NULL OR $chk==NULL OR $cname==NULL OR $typeg==NULL OR $category==NULL OR $hrg==NULL OR $website==NULL )
     {
               $errTyp = "warning";
     ...
     {
             $errTyp = "warning";
             $errMSG = "This registration number is already registered in this website";
     }
     else if($count2 != 1)
     {
         $errTyp = "warning";
         $errMSG = "Sorry Incorrect registration no ...";
     }
     else
     {
           $sql = "INSERT INTO final_hreg(huname,hpass,hosname,state,city,type,ceo,gov,gen,reg,website,email)
           VALUES ('$uname','$spass','$hname','$state','$city','$chk','$cname','$typeg','$category','$hrg','$website','$email')";
          // echo $sql; 
           if(mysqli_query($link,$sql)){
           $emg="You Are Registered.";
           }
...
$hnameErr = $hrgErr = $emailErr = $websiteErr = $unameErr = $stateErr = $cityErr = $cnameErr = $typegErr = $typetErr = $categoryErr = "";
$hname = $hrg = $email = $website = $uname = $state = $city = $cname = $typeg = $typet = $category = "";?>
 ...
 <?php
            if ( isset($errMSG) ) {
                ...
                <a href="reg1d.php"><h3>Go to registration</h3></a>
                <?php
            }
            else if(isset($emg)){
                ?>
